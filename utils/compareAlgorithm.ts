function compareBankHashData(bankData, hashData) {
  ...
  outerloop: for (const bankTrans of bankData) {
    const bankSuF = isNaN(parseFloat(bankTrans.DebVal.trim()))
      ? parseFloat(bankTrans.CredVal.trim())
      : parseFloat(bankTrans.DebVal.trim());
